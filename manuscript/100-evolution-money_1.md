...
On January 3, 2009, Bitcoin was born. Since its creation there has been an Internet Protocol (IP) that attaches new blocks of data to a chain of transactions every 10 minutes. The Bitcoin blockchain stores transactions in the correct chronological order which cannot be altered afterwards, just like time. Just as our heartbeat preserves our bodies, this process is supported by a system that draws its foundations from cryptography, game theory, IT, and decentralized networks that is keeping itself in balance.
...
On 15 August 1971, the United States, lead by President Richard Nixon, unilaterally terminated convertibility of the US dollar to gold, effectively bringing the Bretton Woods system to an end and rendering the dollar a fiat currency. At the same time, many fixed currencies (such as the pound sterling) also became free-floating.
